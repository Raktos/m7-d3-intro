...
$(function() {
  ...
  var paragraphs = div.selectAll('p').data(people, function(d){return d.name});


  // Add a paragraph to the div for each person in your dataset following instructions in the index.html file
  paragraphs.enter().append('p').text(function(d){return d.name}).style({color: function(d){return d.favColor}});
  ...
  // Make sure to specify the `name` attribute as an identifying characteristic in your data-join.
  function update() {
    paragraphs = div.selectAll('p').data(people2, function(d) {return d.name});
    paragraphs.enter().append('p').text(function(d){return d.name}).style({color: function(d){return d.favColor}});
